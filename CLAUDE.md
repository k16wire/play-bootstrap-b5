...
Play-Bootstrap is a Scala/Play Framework library providing input helpers and field constructors for rendering Bootstrap HTML forms. The primary focus is **Play Framework 2.9** with **Bootstrap 5** support.
...
```bash
# For Play 2.9 + Bootstrap 5 (recommended)
...
**Bootstrap version modules** (`play29-bootstrap5`, `play28-bootstrap4`, etc.):
- Depend on corresponding core module
- Contain Twirl templates for specific Bootstrap versions
- Published as `play-bootstrap` with full version suffix (e.g., `1.6.1-P29-B5`)
...
Four field constructor layouts available:
- **Vertical** (`b5.vertical`) - Labels above inputs (default)
- **Horizontal** (`b5.horizontal`) - Labels beside inputs using grid
- **Inline** (`b5.inline`) - Single-line forms
- **Clear** (`b5.clear`) - Raw inputs without wrappers
...
- `play29-bootstrap5/module/app/views/b5/package.scala` - Main form helper definitions