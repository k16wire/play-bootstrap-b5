...
class HelpersSpec extends Specification {
  ...
  val vfc = b5.vertical.fieldConstructor()
  val (colLabel, colInput) = ("col-md-2", "col-md-10")
  val hfc = b5.horizontal.fieldConstructor(colLabel, colInput)
  val ifc = b5.inline.fieldConstructor()
  val cfc = b5.clear.fieldConstructor()

  /**
   * A test field constructor that simply renders the input
   */
  implicit val testFieldConstructor = new B5FieldConstructor {
    val formClass = ""
  ...
  "@inputType" should {
    ...
    "allow setting a custom id" in {
      val body = b5.inputType("text", fooField, Symbol("id") -> "someid").body
    ...
    "allow setting a custom type" in {
      val body = b5.inputType("email", fooField).body
    ...
    "add form-control class as default" in {
      b5.inputType("text", fooField).body must contain("class=\"form-control\"")
    }

    "allow setting additional classes" in {
      b5.inputType("text", fooField, Symbol("class") -> "extra_class").body must contain("class=\"form-control extra_class\"")
    }

    "allow setting a default value" in {
      val body = b5.inputType("text", fooField, Symbol("value") -> "defaultvalue").body
    ...
    "allow being filled with a value" in {
      val body = b5.inputType("text", fooFieldFilled("filledvalue"), Symbol("value") -> "defaultvalue").body
    ...
    "allow setting extra arguments and remove those arguments with false values or with underscored names" in {
      val body = b5.inputType("text", fooField, Symbol("extra_attr") -> "test", Symbol("true_attr") -> true, Symbol("fase_attr") -> false, Symbol("_underscored_attr") -> "test").body
  ...
  def sampleInputTypeBody(theType: String) = b5.inputType(theType, fooField, sampleArgs: _*).body.trim

  "@text" should {
    "be equivalent to inputType with text type" in {
      b5.text(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("text")
    }
  }
  "@password" should {
    "be equivalent to inputType with password type" in {
      b5.password(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("password")
    }
    "not display its value" in {
      b5.password(fooFieldFilled("barValue"), sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("password")
    }
  }
  "@file" should {
    "render file input with form-control class" in {
      val file = b5.file(fooField, sampleArgs: _*).body.trim
      file must contain("""<input type="file" id="someid" name="foo" value=""""")
      file must contain("""class="form-control"""")
  ...
  "@textarea" should {
    ...
    "allow setting a custom id" in {
      val body = b5.textarea(fooField, Symbol("id") -> "someid").body
      val idAttr = "id=\"someid\""
      body must contain(idAttr)
      // Make sure it doesn't have it twice
      body.substring(body.indexOf(idAttr) + idAttr.length) must not contain (idAttr)
    ...
    "add form-control class as default" in {
      b5.textarea(fooField).body must contain("class=\"form-control\"")
    }

    "allow setting additional classes" in {
      b5.textarea(fooField, Symbol("class") -> "extra_class").body must contain("class=\"form-control extra_class\"")
    }

    "allow setting a default value" in {
      val body = b5.textarea(fooField, Symbol("value") -> "defaultvalue").body
  ...
  "@checkbox" should {
    ...
    "allow setting a custom id" in {
      val body = b5.checkbox(boolField, Symbol("id") -> "someid").body
      val idAttr = "id=\"someid\""
      body must contain(idAttr)
      // Make sure it doesn't have it twice
      body.substring(body.indexOf(idAttr) + idAttr.length) must not contain (idAttr)
    ...
    "be unchecked by default" in {
      val body = b5.checkbox(boolField).body
      body must not contain ("checked")
      body must contain("value=\"true\"")
    }

    "allow setting a default custom value" in {
      val body = b5.checkbox(boolField, Symbol("value") -> "bar").body
      body must not contain ("checked")
      body must contain("value=\"bar\"")
    }

    "allow setting a default value for checked attribute" in {
      val body = b5.checkbox(boolField, Symbol("_default") -> true).body
      body must contain("checked")
      body must contain("value=\"true\"")
    }

    "allow setting a default value for checked attribute with a custom value" in {
      val body = b5.checkbox(boolField, Symbol("value") -> "bar", Symbol("_default") -> true).body
      body must contain("checked")
      body must contain("value=\"bar\"")
    }

    "allow being filled with a value" in {
      val body = b5.checkbox(boolFieldFilled(true)).body
      body must contain("checked")
      body must contain("value=\"true\"")
    }

    "allow being filled with a custom value" in {
      val body = b5.checkbox(stringFieldFilled("bar"), Symbol("value") -> "bar").body
      body must contain("checked")
      body must contain("value=\"bar\"")
    }

    "ignore default checked value if it is filled" in {
      val body1 = b5.checkbox(boolFieldFilled(false), Symbol("_default") -> true).body
      body1 must not contain ("checked")
      body1 must contain("value=\"true\"")
      val body2 = b5.checkbox(stringFieldFilled(""), Symbol("value") -> "bar", Symbol("_default") -> true).body
    ...
    "allow setting a forced value for checked attribute (always true)" in {
      val body = b5.checkbox(boolField, Symbol("checked") -> true).body
      body must contain("checked")
      body must contain("value=\"true\"")
    }
    "allow setting a forced value for checked attribute (always false)" in {
      val body = b5.checkbox(boolField, Symbol("checked") -> false).body
      body must not contain ("checked")
      body must contain("value=\"true\"")
    }

    "ignore default and filled checked value if it has forced checked" in {
      val body1 = b5.checkbox(boolFieldFilled(false), Symbol("_default") -> false, Symbol("checked") -> true).body
      body1 must contain("checked")
      body1 must contain("value=\"true\"")
      val body2 = b5.checkbox(boolFieldFilled(true), Symbol("_default") -> true, Symbol("checked") -> false).body
      body2 must not contain ("checked")
      body2 must contain("value=\"true\"")
      val body3 = b5.checkbox(stringFieldFilled(""), Symbol("value") -> "bar", Symbol("_default") -> false, Symbol("checked") -> true).body
      body3 must contain("checked")
      body3 must contain("value=\"bar\"")
      val body4 = b5.checkbox(stringFieldFilled("bar"), Symbol("value") -> "bar", Symbol("_default") -> true, Symbol("checked") -> false).body
      body4 must not contain ("checked")
      body4 must contain("value=\"bar\"")
    }

    "add support to readonly attribute" in {
      val bodyWithoutReadonly = b5.checkbox(boolField, Symbol("value") -> true).body
      ...
      val bodyReadonlyFalse = b5.checkbox(boolField, Symbol("readonly") -> false, Symbol("value") -> true).body
      ...
      val bodyReadonlyTrue = b5.checkbox(boolField, Symbol("readonly") -> true, Symbol("value") -> true).body
    ...
    "render checkbox with form-check classes" in {
      val body = clean(b5.checkbox(boolField, Symbol("_text") -> "theText").body)
      body must contain("""<div class="form-check">""")
      body must contain("""<input type="checkbox" id="foo" name="foo" value="true" class="form-check-input">""")
      body must contain("""<label class="form-check-label" for="foo">theText</label>""")
    }
  }
  ...
  "@radio" should {
    ...
    "allow setting a custom id" in {
      val body = b5.radio(fooField, fruits, Symbol("id") -> "someid").body
    ...
    "be unchecked by default" in {
      b5.radio(fooField, fruits).body must not contain ("checked")
    }

    "allow setting a default value" in {
      val body = b5.radio(fooField, fruits, Symbol("value") -> "B").body
    ...
    "allow being filled with a value" in {
      val body = b5.radio(fooFieldFilled("B"), fruits).body
      val checkedAttr = "checked"
      body must contain(checkedAttr)
      // Make sure it doesn't have it twice
      body.substring(body.indexOf(checkedAttr) + checkedAttr.length) must not contain (checkedAttr)
    ...
    "not be inline by default" in {
      b5.radio(fooField, fruits).body must not contain ("form-check-inline")
    }

    "allow be inline" in {
      b5.radio(fooField, fruits, Symbol("_inline") -> true).body must contain("form-check-inline")
    }

    "add support to readonly attribute" in {
      val bodyWithoutReadonly = b5.radio(fooField, fruits, Symbol("value") -> "B").body
      bodyWithoutReadonly must not contain ("radio-group")
      bodyWithoutReadonly must not contain ("disabled")
      bodyWithoutReadonly must not contain ("<input type=\"hidden\"")

      val bodyReadonlyFalse = b5.radio(fooField, fruits, Symbol("readonly") -> false, Symbol("value") -> "B").body
      ...
      val bodyReadonlyTrue = b5.radio(fooField, fruits, Symbol("readonly") -> true, Symbol("value") -> "B").body
    ...
    "render radio with form-check classes" in {
      val body = clean(b5.radio(fooField, fruits).body)
      body must contain("""<div class="form-check">""")
      body must contain("""<input type="radio" id="foo_""")
      body must contain("""class="form-check-input"""")
      body must contain("""<label class="form-check-label"""")
    }
  }
  ...
  "@select" should {
    ...
    "allow setting a custom id" in {
      val body = b5.select(fooField, fruits, Symbol("id") -> "someid").body
      body must contain("id=\"someid\"")
    }

    "add form-select class as default" in {
      b5.select(fooField, fruits).body must contain("class=\"form-select\"")
    }

    "allow setting additional classes" in {
      b5.select(fooField, fruits, Symbol("class") -> "extra_class").body must contain("class=\"form-select extra_class\"")
    }

    "be unselected by default" in {
      b5.select(fooField, fruits).body must not contain ("selected")
    }

    "allow setting a default value" in {
      val body = b5.select(fooField, fruits, Symbol("value") -> "B").body
    ...
    "allow being filled with a value" in {
      val body = b5.select(fooFieldFilled("B"), fruits).body
      val selectedAttr = "selected"
      body must contain(selectedAttr)
      // Make sure it doesn't have it twice
      body.substring(body.indexOf(selectedAttr) + selectedAttr.length) must not contain (selectedAttr)
    ...
    "add support to readonly attribute" in {
      val bodyWithoutReadonly = b5.select(fooField, fruits, Symbol("value") -> "B").body
      bodyWithoutReadonly must not contain ("<div class=\"select-group\">")
      bodyWithoutReadonly must not contain ("disabled")
      bodyWithoutReadonly must not contain ("<input type=\"hidden\"")

      val bodyReadonlyFalse = b5.select(fooField, fruits, Symbol("readonly") -> false, Symbol("value") -> "B").body
      bodyReadonlyFalse must contain("<div class=\"select-group\">")
      bodyReadonlyFalse must not contain ("disabled=\"true\"")
      bodyReadonlyFalse must contain("<input type=\"hidden\" name=\"foo\" value=\"B\" disabled/>")

      val bodyReadonlyTrue = b5.select(fooField, fruits, Symbol("readonly") -> true, Symbol("value") -> "B").body
    ...
    "allow multiple" in {
      val body = b5.select(fooField, fruits, Symbol("multiple") -> true, Symbol("value") -> "P,B").body
  ...
  "@hidden" should {
    "be rendered correctly" in {
      val body = clean(b5.hidden("testName", "testValue", Symbol("foo") -> "bar").body)
      body must be equalTo """<input type="hidden" name="testName" value="testValue" foo="bar">"""
    }
    "with Field object" in {
      val body = clean(b5.hidden(fooField, Symbol("value") -> "testValue", Symbol("foo") -> "bar").body)
      body must be equalTo """<input type="hidden" name="foo" value="testValue" foo="bar">"""
    }
    "with filled Field object" in {
      val body = clean(b5.hidden(fooFieldFilled("filledValue"), Symbol("value") -> "testValue", Symbol("foo") -> "bar").body)
      body must be equalTo """<input type="hidden" name="foo" value="filledValue" foo="bar">"""
    }
  }
  "@hiddens" should {
    "be rendered correctly" in {
      val body = clean(b5.hiddens("fooId" -> 1L, "barId" -> 2L).body)
  ...
  "@color" should {
    "be equivalent to inputType with date type" in {
      b5.color(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("color")
    }
  }
  "@date" should {
    "be equivalent to inputType with date type" in {
      b5.date(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("date")
    }
  }
  "@datetime" should {
    "be equivalent to inputType with date type" in {
      b5.datetime(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("datetime")
    }
  }
  "@datetimeLocal" should {
    "be equivalent to inputType with date type" in {
      b5.datetimeLocal(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("datetime-local")
    }
  }
  "@email" should {
    "be equivalent to inputType with email type" in {
      b5.email(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("email")
    }
  }
  "@month" should {
    "be equivalent to inputType with date type" in {
      b5.month(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("month")
    }
  }
  "@number" should {
    "be equivalent to inputType with date type" in {
      b5.number(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("number")
    }
  }
  "@range" should {
    "be equivalent to inputType with date type" in {
      b5.range(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("range")
    }
  }
  "@search" should {
    "be equivalent to inputType with date type" in {
      b5.search(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("search")
    }
  }
  "@tel" should {
    "be equivalent to inputType with date type" in {
      b5.tel(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("tel")
    }
  }
  "@time" should {
    "be equivalent to inputType with date type" in {
      b5.time(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("time")
    }
  }
  "@url" should {
    "be equivalent to inputType with date type" in {
      b5.url(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("url")
    }
  }
  "@week" should {
    "be equivalent to inputType with date type" in {
      b5.week(fooField, sampleArgs: _*).body.trim must be equalTo sampleInputTypeBody("week")
    }
  }

  "@formGroup" should {

    def testFormGroup(args: (Symbol, Any)*)(fc: b5.B5FieldConstructor, msgsProv: MessagesProvider) =
      clean(b5.freeFormGroup(args)(innerArgs => Html("<content>"))(fc, msgsProv).body)
    ...
    "vertical: show label" in {
	  ...
	  <div class="mb-3 theClass" id="theId">
	  	<label class="form-label">theLabel</label>
	  	<content>
	  </div>
	  """)
    }
    ...
    "horizontal: show label" in {
	  ...
	  <div class="mb-3 row theClass" id="theId">
    ...
    "inline: show label" in {
	  ...
	  <div class="mb-3 col-12 theClass" id="theId">
	  	<label class="form-label">theLabel</label>
      <content>
	  </div>
	  """)
    ...
    "get the inner arguments for the content" in {
      val body = b5.freeFormGroup(Seq(Symbol("_class") -> "theClass", Symbol("_underscored") -> "underscored", Symbol("foo") -> "foo"))(innerArgsMap => Html(innerArgsMap.toSeq.map(a => s"""${a._1.name}="${a._2.toString}"""").mkString("<content ", " ", ">")))(vfc, msgsProv).body
  ...
  "@free" should {
    "be rendered correctly" in {
      clean(b5.free(Symbol("foo") -> "fooValue")(Html("<content>"))(vfc, msgsProv).body) must be equalTo clean(b5.freeFormGroup(Seq(Symbol("foo") -> "fooValue"))(_ => Html("<content>"))(vfc, msgsProv).body)
    }
  }

  "@static" should {

    "render with form-control-plaintext class as default" in {
      b5.static("theLabel")(Html("theText"))(vfc, msgsProv).body must contain("<p class=\"form-control-plaintext\">theText</p>")
    }

    "allow setting additional classes" in {
      b5.static("theLabel", Symbol("class") -> "extra_class")(Html("theText"))(vfc, msgsProv).body must contain("<p class=\"form-control-plaintext extra_class\">theText</p>")
    }

    "allow setting extra arguments and remove those arguments with false values or with underscored names" in {
      val body = b5.static("theLabel", Symbol("extra_attr") -> "test", Symbol("true_attr") -> true, Symbol("fase_attr") -> false, Symbol("_underscored_attr") -> "test")(Html("theText"))(vfc, msgsProv).body
      body must contain("extra_attr=\"test\"")
      body must contain("true_attr=\"true\"")
      body must not contain ("false_attr=\"false\"")
      body must not contain ("_underscored_attr=\"test\"")
  ...
  "@buttonType" should {
    ...
    def buttonTypeBody(args: (Symbol, Any)*) = b5.buttonType(sampleType, args: _*)(Html(sampleContent))(vfc, msgsProv).body
  ...
  def sampleButtonTypeBody(theType: String) = b5.buttonType(theType, sampleArgs: _*)(Html("content"))(vfc, msgsProv).body.trim

  "@submit" should {
    "be equivalent to buttonType with submit type" in {
      b5.submit(sampleArgs: _*)(Html("content"))(vfc, msgsProv).body.trim must be equalTo sampleButtonTypeBody("submit")
    }
  }
  "@reset" should {
    "be equivalent to buttonType with reset type" in {
      b5.reset(sampleArgs: _*)(Html("content"))(vfc, msgsProv).body.trim must be equalTo sampleButtonTypeBody("reset")
    }
  }
  "@button" should {
    "be equivalent to buttonType with button type" in {
      b5.button(sampleArgs: _*)(Html("content"))(vfc, msgsProv).body.trim must be equalTo sampleButtonTypeBody("button")
    }
  }
  ...
  "@inputWrapped" should {
    ...
    "be equivalent to inputType for an empty wrapper" in {
      val bodyInputType = clean(b5.inputType("text", fooField, Symbol("id") -> "someid").body)
      val body = clean(b5.inputWrapped("text", fooField, Symbol("id") -> "someid")(x => x).body)
    ...
    "wrap the input" in {
      ...
      val body = clean(b5.inputWrapped("text", fooField, Symbol("id") -> "someid")(input => wrap(input)).body)
  ...
  "@multifield" should {
    ...
    def multifield(form: Form[(String, String)], globalArgs: Seq[(Symbol, Any)] = Seq(), fieldsArgs: Seq[(Symbol, Any)] = Seq())(fc: b5.B5FieldConstructor, msgsProv: MessagesProvider) =
      clean(b5.multifield(form("foo"), form("bar"))(globalArgs, fieldsArgs)(cfc => Html(testInputsString))(fc, msgsProv).body)
    ...
    "have the basic structure" in {
      ...
      body must contain("class=\"mb-3")
      body must not contain ("has-danger")
      body must contain("<label class=\"form-label\">theLabel</label>")
    ...
    "allow setting extra classes form-group" in {
      fooMultifield(Symbol("_class") -> "extra_class another_class") must contain("class=\"mb-3 extra_class another_class")
    }

    "show label" in {
      multifield(fooForm, Seq(Symbol("_label") -> "fooLabel"))(vfc, msgsProv) must contain("<label class=\"form-label\">fooLabel</label>")
    ...
    "render validation states" in {
      def withStatus(status: String) = contain(s"""<div class="mb-3 has-$status"""")
      def testStatus(status: String, withFieldsArgs: Boolean, args: (Symbol, Any)*) = {
        val test = if (withFieldsArgs)
          clean(b5.multifield(fooForm("foo"))(globalArgs = Seq(), fieldsArgs = args)(cfc => b5.text(fooForm("foo"), args: _*))(vfc, msgsProv).body)
        else
          clean(b5.multifield(fooForm("foo"))(globalArgs = args, fieldsArgs = Seq())(cfc => b5.text(fooForm("foo"), args: _*))(vfc, msgsProv).body)
