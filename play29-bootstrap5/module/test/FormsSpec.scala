...
class FormsSpec extends Specification {
  ...
  def fooFormBody(args: (Symbol, Any)*)(fc: b5.B5FieldConstructor) = b5.form(fooCall, args: _*)(Html(testContentString))(fc).body
  ...
  "@form" should {
    ...
    "add default class for each field constructor" in {
      // Bootstrap 5 doesn't use form-vertical, form-horizontal, etc. classes anymore
      fooFormBody()(vfc) must contain("role=\"form\"")
      fooFormBody()(hfc) must contain("role=\"form\"")
      fooFormBody()(ifc) must contain("class=\"row row-cols-lg-auto g-3 align-items-center")
      fooFormBody()(cfc) must contain("role=\"form\"")
    }

    "allow setting custom class" in {
      fooFormBody(Symbol("class") -> "customClass")(vfc) must contain("class=\"customClass\"")
    }

    "allow disabling default class" in {
      fooFormBody(Symbol("_disableDefaultClass") -> true)(vfc) must not contain ("class=\"\"")
